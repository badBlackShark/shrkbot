...
5. **Plugin config pages** (shared template) — **explicit Save, not auto-save** (see SAVE MODEL above): one form per page, a Save button commits all edits at once; toggles here are `Components::Toggle` fields (`submit_on_change: false`). Reuse the dashboard's Turbo-Stream save-feedback (toast) on Save.
