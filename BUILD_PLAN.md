...
**Design delivered + unpacked at `docs/design/`** (`README.md` = system guide + voice; `ui_kits/dashboard/index.html` = click-through kit with EVERY screen; `tailwind/theme.css` = the v4 `@theme` block; `tokens/*` = source CSS custom props incl. dark + motion; `foundations/*` = specimens; self-hosted woff2 under `fonts/`+`packages/`). The HTML mockups are throwaway prototype wiring (Google-Fonts/Lucide/Tailwind-Play CDNs); the **token files + `@theme` are the production spec**. Port the kit ~1:1 into Phlex + Stimulus; dropdowns via Tom Select.

**SAVE MODEL (revised 2026-06-22, supersedes the blanket "auto-save via Turbo" of design #21):** *standalone* settings save instantly — a single toggle that owns its form and submits on change (the dashboard's plugin-enable + `force_dm_reminders`), responding with a Turbo Stream that re-renders the control in place + a toast. *Config pages* (Logging/Roles/Welcomes) do **NOT** instant-save: they collect all edits in one form behind an **explicit Save button**, so a user can stage a full configuration before it goes live. On/off settings on those pages are still toggle switches, but as plain form fields (`Components::Toggle` with `submit_on_change: false`, no own form) saved by the page's Save.
...
4. **Server dashboard** — the 3 toggleable plugins as rows (icon tile, status badge Enabled/Needs-setup/Off, Configure entry, enable toggle) + **reminders as an always-on row** (locked/disabled toggle + hover note "always enabled to preserve DM functionality"). `force_dm_reminders` is NOT here — it moved to the reminders config page (chunk 5).
4.5. **Web infrastructure PR (do BEFORE chunk 5; from PR #61 review).** → DELIVERED in PR #62 (`feat/web-infrastructure`, pending review). The dashboard shipped these inline because it was the first consumer; chunk 5 multiplies the consumers, so extract them once, with the second consumer in view, rather than guessing the abstraction now:
   - **Authorization/scoping concern.** Today `Servers::PluginsController#authorize_server` gates mutations against `session[:authorized_server_ids]` (the servers the user proved manageable via Discord) — this is NOT spoofable, but every server-scoped controller will repeat it. Extract to a shared concern (e.g. `before_action :require_manageable_server` setting `@server_configuration`). NOTE: our authz is **Discord-derived + session-cached**, not a DB relationship, so the work-codebase `Scopes.plugins_for(user)` AR-scope pattern doesn't map — there's no `user.manageable_servers` association to scope on (by design #12, we don't persist a user→server map). The reusable unit is the before_action/concern, not an AR scope.
   - **Turbo-save mechanism.** Turbo Stream rendering moved out of controller helpers into the standard Rails `action.turbo_stream.erb` view template (the controller sets ivars + `respond_to`; the template's `turbo_stream.replace/append` lines render our Phlex components as content) — all the Turbo wiring in one place, separate from controller logic. Config-page saves reuse the same convention, handling BOTH outcomes: success (re-render control/region + success toast) AND failure (re-render the form/region with **inline validation errors**, just more `turbo_stream.*` lines), since chunk-5 config forms surface enable-without-prereqs as a field error (see chunk 5).
5. **Plugin config pages** (shared template) — **explicit Save, not auto-save** (see SAVE MODEL above): one form per page, a Save button commits all edits at once; toggles here are `Components::Toggle` fields (`submit_on_change: false`). Reuse the chunk-4.5 turbo-save mechanism. **DECIDED (PR #61 review):** the **enable/disable toggle lives ON the config form too** (as a staged field, not instant) — so a user sets the channel AND enables in one Save, and enabling-without-prereqs renders as an **inline form error** next to the offending field rather than a toast. The form therefore commits two models (the plugin's settings + its `PluginActivation`), via a **per-plugin "configure" op** (e.g. `Ops::Logging::Configure`) that, in one transaction: saves settings FIRST, then toggles the activation (so the enable prerequisite check sees the just-saved channel), and returns aggregated, field-mapped errors for re-render. NOT a generic `Plugins::Update` — `Plugins::Toggle` stays for the dashboard's instant on/off path. (Settings are still their own model; the config op orchestrates settings-save + toggle.) The chunk-4.5 turbo-save mechanism must therefore support **re-rendering a form with validation errors**, not just success toasts.
   ...
   - **Reminders** — config page for the otherwise-toggleless global plugin: the server-level **`force_dm_reminders`** toggle (moved off the dashboard) + explain `/remind`. Needs `Ops::ServerConfiguration::Update` (the force_dm setter, removed from chunk 4 — rebuild here with its caller).
...
- **GATE:** toggle a plugin, set a log channel, configure a role set (incl. grey-out) — persist + validate; dark mode + the dashboard's instant toggles + config-page explicit Save + enable-gate work end-to-end.
