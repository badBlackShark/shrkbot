...
RSpec.describe Reminders::Remind do
  ...
  def success_result
    Ops::ApplicationOperation::Result.new(true, double(remind_at: Time.at(1_000)), [])
  end

  it "calls Ops::CreateReminder with the event's options and confirms" do
    expect(Ops::CreateReminder).to receive(:call)
  ...
  it "maps deliver=dm to deliver_via_dm: true" do
    ...
    expect(Ops::CreateReminder).to receive(:call).with(hash_including(deliver_via_dm: true)).and_return(success_result)
    described_class.new(event).execute
  end

  it "surfaces the operation's failure message" do
    allow(Ops::CreateReminder).to receive(:call).and_return(Ops::ApplicationOperation::Result.new(false, nil, ["bad duration"]))
