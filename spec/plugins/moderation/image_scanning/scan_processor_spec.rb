...
RSpec.describe Moderation::ImageScanning::ScanProcessor do
  ...
  let(:verdict) { Moderation::ImageScanning::Verdict.new(action: :allow, risk: 0.0, reasons: []) }

  before do
    allow(Moderation::ImageScanning::ImageDownload).to receive(:call).and_return("bytes")
    allow(Moderation::ImageScanning::Ocr::Client).to receive(:new).and_return(client_double)
    ...
    allow(Moderation::ImageScanning::PhashIndex).to receive(:lookup).and_return(state)
    allow(Ops::Moderation::Phashes::MarkSeen).to receive(:call)
    allow(Moderation::ImageScanning::Classifier).to receive(:call).and_return(verdict)
    allow(Moderation::ImageScanning::VerdictExecutor).to receive(:call)
  end
  ...
  context "when the phash is unknown (state :none)" do
    ...
    it "runs the OCR scan and classifies with the unknown hash state" do
      ...
      expect(Moderation::ImageScanning::Classifier).to have_received(:call).with(
        ocr_text: "won usdt",
        hash_state: :none,
        signals:,
        settings:,
  ...
  context "when the phash is this guild's confirmed scam (state :own_confirmed)" do
    ...
    it "touches last_seen with the hex and runs the executor" do
      ...
      expect(Moderation::ImageScanning::VerdictExecutor).to have_received(:call).with(verdict:, context:, phash: hex, hash_state: :own_confirmed, image_bytes: "bytes")
    end
  end
  ...
  context "when the phash was dismissed here (state :own_dismissed)" do
    ...
    it "touches last_seen then short-circuits" do
      ...
      expect(Ops::Moderation::Phashes::MarkSeen).to have_received(:call).with(phash_hex: hex)
      expect(Moderation::ImageScanning::Classifier).not_to have_received(:call)
      expect(Moderation::ImageScanning::VerdictExecutor).not_to have_received(:call)
    end
  end

  context "when the OCR scan raises an Ocr::Error" do
    before do
      allow(client_double).to receive(:scan).and_raise(Moderation::ImageScanning::Ocr::Error, "boom")
    end
  ...
  context "when the phash call raises an Ocr::Error" do
    before do
      allow(client_double).to receive(:phash).and_raise(Moderation::ImageScanning::Ocr::Error, "boom")
    end
  ...
  context "when the attachment download returns a non-success status" do
    before do
      allow(Moderation::ImageScanning::ImageDownload).to receive(:call).and_raise(Moderation::ImageScanning::Ocr::Error, "attachment download failed: 404")
    end
  ...
  context "when the attachment download connection fails" do
    before do
      allow(Moderation::ImageScanning::ImageDownload).to receive(:call).and_raise(Moderation::ImageScanning::Ocr::Error, "no dns")
    end
