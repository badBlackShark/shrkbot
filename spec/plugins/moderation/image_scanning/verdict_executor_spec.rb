...
RSpec.describe Moderation::ImageScanning::VerdictExecutor do
  ...
  let(:reasons) do
    [
      Moderation::ImageScanning::Reason.new(key: :new_account, weight: 2, detail: 3),
      Moderation::ImageScanning::Reason.new(key: :rule, weight: 3, detail: "promo code")
    ]
  end
  ...
  context "when the action is :flag_for_review" do
    ...
    context "with keyword, foreign-hash, and fractional-risk reasons" do
      let(:reasons) do
        [
          Moderation::ImageScanning::Reason.new(key: :custom_keywords, weight: 4, detail: 2),
          Moderation::ImageScanning::Reason.new(key: :foreign_confirmed, weight: 0),
          Moderation::ImageScanning::Reason.new(key: :no_role, weight: 0.5)
        ]
      end
      let(:verdict) { Moderation::ImageScanning::Verdict.new(action:, risk: 4.5, reasons:) }
