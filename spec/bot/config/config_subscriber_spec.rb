...
RSpec.describe Bot::ConfigSubscriber do
  ...
  describe "#start" do
    ...
    it "subscribes on the config channel and routes each message to #handle" do
      ...
      allow(Redis).to receive(:new).with(url: Bot::Config.redis_url).and_return(redis)
      allow(redis).to receive(:subscribe).with(Bot::ConfigBus::CHANNEL).and_yield(on).and_raise(StopIteration)
  ...
  describe "#handle" do
    ...
    context "with malformed JSON" do
      ...
      it "reports the error instead of crashing the listener" do
        ...
        expect { subscriber.handle(payload) }.not_to raise_error
        expect(Bot::OwnerNotifier).to have_received(:report)
      end
    end
