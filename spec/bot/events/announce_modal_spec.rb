...
RSpec.describe Bot::AnnounceModal do
  subject(:handle) { described_class.new(event).handle }

  let(:event) { double("event", bot: double("bot"), respond: nil, defer: nil, edit_response: nil) }

  before do
    allow(event).to receive(:value).with(Bot::Commands::Announce::INPUT_ID).and_return("the announcement")
    allow(Bot::Registry).to receive(:all).and_return([event.bot])
  end

  context "when the bot owner submits" do
    before do
      allow(Bot::CommandPermissions).to receive(:permitted?).and_return(true)
    end

    it "defers, broadcasts the submitted text, and reports a summary" do
      expect(event).to receive(:defer).with(ephemeral: true)
      expect(Bot::OwnerBroadcast).to receive(:call)
        .with(bots: [event.bot], content: "the announcement")
        .and_return(Bot::OwnerBroadcast::Result.new(owner_count: 3, sent: 3, server_count: 5))
  ...
  context "when someone other than the bot owner submits" do
    before do
      allow(Bot::CommandPermissions).to receive(:permitted?).and_return(false)
    end

    it "rejects without broadcasting" do
      expect(Bot::OwnerBroadcast).not_to receive(:call)
