...
RSpec.describe Bot::ServerBackfill do
  ...
  it "syncs every server the bot is already in" do
    expect(Bot::GuildMetadata).to receive(:sync).with(server_a, bot)
    expect(Bot::GuildMetadata).to receive(:sync).with(server_b, bot)
    handle
  end
end
