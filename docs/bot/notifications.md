...
Notifications are written by bot-side ops (`Ops::Notification::Create`) when a
...
**Ops:** `Ops::Notification::Dismiss` sets `dismissed_at` on a single
notification. `Ops::Notification::MarkRead` bulk-updates `read_at` via
