...
`Components::Toggle` fields). Both paths render through `Components::TurboStream`
(see architecture.md "Web"): the controller declares stream operations
(`.replace(target, component)` / `.append("toasts", toast)`) and renders the
primitive, so the stream markup stays in the view layer. Success re-renders the
control plus a toast; a config-form failure re-renders the form region with inline
errors — same primitive, different components. The remaining config-form controls
