...
so every later boot costs two calls: the application lookup and one emoji
listing. Unlike `db:prepare`, a failure here does not stop the boot: it logs and
carries on, because a Discord outage must not keep the queue down. Those seven
flags then render empty until a boot that reaches Discord.
