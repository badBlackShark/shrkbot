# Some commands to go with SSBLogger
module LoggerCommands
  ...
  command :setLogChannel, attrs do |event, *args|
    ...
    DB.update_value("ssb_server_#{event.server.id}".to_sym, :log_channel, channel.id)
  ...
  command :logChannel?, attrs do |event|
    log_channel = DB.read_value("ssb_server_#{event.server.id}".to_sym, :log_channel)
