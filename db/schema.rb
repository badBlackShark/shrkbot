...
ActiveRecord::Schema[8.1].define(version: 2026_07_25_231649) do
  ...
  create_table "image_scanning_settings", id: :string, default: -> { "('iss_'::text || gen_random_uuid())" }, force: :cascade do |t|
    ...
    t.check_constraint "confirmed_punishment::text = ANY (ARRAY['none'::character varying::text, 'timeout'::character varying::text, 'kick'::character varying::text, 'ban'::character varying::text])", name: "image_scanning_settings_confirmed_punishment_check"
  ...
  create_table "moderation_verdicts", id: :string, default: -> { "('mvr_'::text || gen_random_uuid())" }, force: :cascade do |t|
    ...
    t.check_constraint "action::text = ANY (ARRAY['flag_for_review'::character varying::text, 'remove'::character varying::text])", name: "moderation_verdicts_action_check"
    t.check_constraint "punishment::text = ANY (ARRAY['none'::character varying::text, 'timeout'::character varying::text, 'kick'::character varying::text, 'ban'::character varying::text])", name: "moderation_verdicts_punishment_check"
  end
